...
async function check_regedit() {
  ...
  if (
    SOFTWARE['HKLM\\SOFTWARE'].hasOwnProperty('keys') &&
    SOFTWARE['HKLM\\SOFTWARE'].keys.indexOf('ChunKun') > -1
  ) {
    var COMPANY = await queue_company()
    if (
      COMPANY['HKLM\\SOFTWARE\\ChunKun'].hasOwnProperty('values') &&
      COMPANY['HKLM\\SOFTWARE\\ChunKun'].values.hasOwnProperty(name)
    ) {
      return false
    } else {
